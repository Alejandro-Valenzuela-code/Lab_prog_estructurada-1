# ----------------------------------------------
#    Recorrido del caballo (Backtracking) (UNA SOLUCIÓN)
# ----------------------------------------------

MAX = int(input("ingrese la dimensión del tablero(ejemplo: 5): "))  # Tamaño del tablero
...
''' Módulo: final '''
# Revisa si todas las casillas del tablero están ocupadas
def final(tablero):
    for i in range(MAX):
        for j in range(MAX):
            if tablero[i][j] == 0:
...
def solucion(tablero):
    ...
    solucion = False        # comprobar si se encontró solución
