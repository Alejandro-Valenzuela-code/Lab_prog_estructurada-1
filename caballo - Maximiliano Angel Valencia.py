...
def solucion(tablero):
    ...
    while(candidato <= 8 and not solucion):  # Bucle principal: intenta candidatos 1..8 hasta encontrar solución
        if(valida(tablero, candidato, x, y)):  # Si el candidato actual es válido desde la posición x,y
            ...
            if(final(tablero)):  # Si el tablero quedó completo (no hay ceros)
                solucion = True  # Marca que se encontró una solución
            else:
                tablero_aux[x][y] = candidato; x = nx; y = ny; contador = contador + 1  # Guarda el candidato usado, avanza a nx,ny y aumenta contador
