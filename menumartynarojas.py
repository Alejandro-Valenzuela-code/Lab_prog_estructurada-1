...
if numero==1:
    crear_archivo("martyfile.txt")
else:
    if numero==2:
        eliminar_archivo("martyfile.txt")
    else:
        if numero==3:
            contenido = input ("Ingrese contenido a escribir:")
            escribir_archivo("martyfile.txt", contenido)
        else:
            if numero==4:
                leer_archivo("martyfile.txt")
            else:
                if numero==5:
                    agregar_archivo("martyfile.txt")
                else:
                    exit
